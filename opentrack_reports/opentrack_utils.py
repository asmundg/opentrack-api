...
TRACK_EVENT_CODES = ['60', '100', '200', '400', '600', '800', '1500', '3000', '5000', '10000', 
                     '60H', '80H', '100H', '110H', '200H', '300H', '400H', '3000SC', 
                     '4x100', '4x200', '4x400']
...
def validate_events(data: dict[str, Any], strict_mode: bool = True) -> list[dict[str, Any]]:
    """
    ...
    if 'events' not in data:
        if strict_mode:
            raise ValueError("Data must be a dict with 'events' key from OpenTrack JSON")
        return []
    ...
    if unrecognized_events:
        ...
        for event in unrecognized_events:
            error_details.append(f"  - {event['name']} (Code: '{event['eventCode']}', ID: {event['eventId']})")
        
        error_message = f"Found {len(unrecognized_events)} unrecognized event(s):\n" + "\n".join(error_details)
